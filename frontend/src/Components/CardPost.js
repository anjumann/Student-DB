...
const CardPost = (props) => {
    ...
    const { name, usn, likes, description, image } = props
    const [Modal, setModal] = useState(false)
    const postImage = image || "https://source.unsplash.com/301x301/?random"
    const [authsesstion, setAuthSession] = useRecoilState(sessionAuth);
    const [curUser, setCurUser] = useState(false)

    useEffect(() => {
        if (authsesstion !== null) {
            if (usn === authsesstion?.usn) {
                setCurUser(true)
            }
        }
    }, [])
    ...
    return (
        ...
        <>
            <div className="shadow-md sm:w-96 bg-none backdrop-blur-2xl backdrop-brightness-50 rounded-xl text-gray-100 cursor-pointer">
                <div className="flex items-center justify-between p-3">
                    <div className="flex items-center space-x-2">
                        ...
                        <div className="-space-y-1">
                            ...
                            <span className="inline-block text-xs leading-none text-gray-400">{usn}</span>
                        </div>
                    </div>
                    {
                        curUser ? (
                            <button title="Open options" onClick={() => { setModal(true) }} type="button">
                ...
                <img src={postImage} alt="" className="object-cover object-center w-full h-72 bg-gray-500" />
                <div className="p-3">
                    ...
                    <div className="space-y-3">
                        <p className="text-sm">
                            <span className="text-base font-semibold">{name}{" "}</span>{description}
                        </p>
            ...
            {Modal ?
                <ModalPost usn={usn} {...{ Modal, setModal }} />
                : ''}
        </>
    )
}
